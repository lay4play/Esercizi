...
int binariaRic(int inizio, int fine, int numero){
  int medio;
  if (inizio > fine)
    return -1;                     // numero non presente 
  else {
    medio = (inizio + fine) / 2;
    if (numero == numeri[medio])
      return medio;                // e' proprio il medio 
    else
      if (numero > numeri[medio])  // nella metà superiore
        return binariaRic(medio + 1, fine, numero);
      else                         // nella metà inferiore
        return binariaRic(inizio, medio - 1, numero);
  }
}
...
int main(void) {
  ...
  cin >> cerca;
  trovato = binariaRic(0, TANTI, cerca);
  if (trovato == -1)
